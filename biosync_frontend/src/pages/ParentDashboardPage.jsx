...
export default function ParentDashboardPage() {
  ...
  const handleGenerate = async (e) => {
    ...
    setLoading(true);
    setData(null);
    setSnapshot({ regNo, year, className, startDate, endDate });

    try {
      const payload = { reg_no: regNo, year: year, class_name: className, start_date: startDate, end_date: endDate };
      // Passing signal assuming api wrapper supports it, if not it just ignores it.
      const response = await api.post('/parent/student-attendance', payload, { signal: abortControllerRef.current.signal });
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#050505', color: '#fff', fontFamily: 'Inter, sans-serif', overflowX: 'auto' }}>
      ...
      <main style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem', boxSizing: 'border-box' }}>
        ...
        {data && snapshot && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ background: 'rgba(96, 165, 250, 0.1)', border: '1px solid #60a5fa', padding: '0.75rem', borderRadius: '4px', fontSize: '0.85rem', color: '#60a5fa', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <CheckCircle size={16} style={{ flexShrink: 0 }} /> 
              <span>Generated for: <strong>{snapshot.regNo}</strong> | Year {snapshot.year} | {snapshot.className} | {snapshot.startDate} to {snapshot.endDate}</span>
            </div>
